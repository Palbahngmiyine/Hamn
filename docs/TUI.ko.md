...
프로필 Docker·Kubernetes에는 이 CLI 설치를 요구하지 않습니다. 외부 Docker
`--context` 요청은 Docker CLI가 필요합니다.
...
| `m`, `?` | 대상에 맞는 작업 메뉴, 명령 도움말 |
| `p`, `R`, `[`, `]` | 갱신 일시정지, 즉시 갱신, 간격 축소·확대(1–60초) |
| `b`, `f`, `F` | 최근 대상, 즐겨찾기 전환, 즐겨찾기 대상 |
| Ctrl+Alt+B, Ctrl+Alt+S | CLI를 유지하며 탐색으로 복귀, 세션 목록 |
| Tab, `,` | 영역 전환, 기본 영역 변경 |
...
`exec` 안에서 셸을 명시적으로 실행하세요. 큰따옴표 안의 JSONPath·정규식에서
`\n`, `\t`, `\.`는 그대로 전달하며, 역슬래시 뒤의 줄바꿈은 같은 인자를 이어 씁니다.
구조화 조회 출력은 16 MiB로 제한하며
초과하면 오류를 표시합니다.
구조화 조회가 끝나거나 취소되면 같은 프로세스 그룹에서 실행한 CLI 보조 프로세스도
종료합니다. CLI가 먼저 종료된 뒤 출력 파이프를 계속 잡고 있는 경우도 포함합니다.
...
다시 읽는 동안에도 입력과 종료를 처리합니다. 다른 화면이나 명령으로 이동하면
재조회를 취소하므로 늦게 도착한 결과가 새 선택을 덮어쓰지 않습니다.
...
실패한 변경 작업의 상태와 로그는 다른 작업 영역을 보고 있어도 그 작업을 시작한
영역에 남으며, `outcomeUnknown`이면 원래 대상도 `!`에 남깁니다. 확정 실패는
결과 불명으로 바꾸지 않습니다. 종료 시 터미널을 복원한 뒤 이 결과를 다시 출력합니다.
...
중단된 게스트 배포는 시작을 계속하기 전에 복구합니다. 완전하고 소유권이 맞는 백업
하나만 rollback한 뒤 배포를 다시 시도합니다. 모호하거나 불완전한 백업은 점검을 위해
...
저장합니다. 기존 version 1 문서와 호환되는 `recentTargets`·`favorites`도 선택적으로
저장합니다. 쓰기 작업은 전용 `~/.hamn/tui.lock`으로 보호하며 동시 쓰기는 UI를
막거나 다른 변경을 덮어쓰지 않고 busy·재시도 오류로 알려줍니다. 활성 세션은
실행 중인 TUI에만 존재하고 종료 후 복원하지 않습니다.


## 장애 진단·갱신·세션

갱신·정렬 변경 시 선택은 리소스의 고유 식별자를 따릅니다. 객체가 사라지거나 교체되고
식별자가 중복되면 선택을 해제합니다. 붙여넣은 검색어도 일반 입력과 같은 경로로
반영합니다. Docker volume·Compose project·Hamn profile 목록은 불변 UID 대신
이름을 제공하므로 조회 사이에 같은 이름으로 재생성된 객체는 구분할 수 없습니다.
Pod는 Ready·재시작 수·대기/종료 원인, Event는 종류·원인·메시지,
Node는 준비 상태와 활성 condition을 표시합니다. Deployment·StatefulSet·DaemonSet은
준비·갱신·가용 수를 표시하며, 다른 resource kind는 일반 상태 표를 사용합니다.

`m`은 적용 가능한 작업을 표시하고 `l`은 기본 tail 200줄·timestamps 로그 메뉴를
엽니다. Pod의 container·previous 로그를 고를 수 있으며 직접 입력한 명령의 인자는
바꾸지 않습니다. `:compose ls`에서 프로젝트를 선택하면 해당 label의 컨테이너로
이동합니다. workload는 selector의 Pods, Pod는 UID에 연결된 Events로 이동합니다.
`:get certificates.cert-manager.io` 같은 custom resource도 표로 읽을 수 있으며
알 수 없는 종류에 선택 기반 변경 단축키를 제공하지 않습니다.

기본 갱신 간격은 2초, 조회 제한 시간은 30초입니다. `p`로 멈추고 `R`로 즉시
조회하며 `[`·`]`로 간격을 조정합니다. 실패 시 최대 60초까지 간격을 늘립니다.
상단에서 갱신·일시정지·오류·마지막 성공 시각을 구분합니다.
`:refresh-timeout 30`으로 제한 시간을 1–300초 사이로 바꿀 수 있습니다.
16 MiB 응답 제한에 걸리면 `--namespace`, `--selector`, `--field-selector`로 서버 조회
범위를 줄이세요. `/`는 이미 가져온 행만 필터링합니다.

Ctrl+Alt+B는 CLI를 계속 실행하면서 탐색으로 돌아갑니다. Ctrl+Alt+S의 세션 목록에서
대상·상태를 보고 Enter로 다시 열거나 `d`로 해당 세션을 끝냅니다. 일반 Tab 등은
활성 CLI에 그대로 전달합니다. 앱 종료는 소유 세션을 정리하고 독립 VM은 멈추지 않습니다.
명령 입력의 Up/Down은 메모리 내 이력을 사용합니다. 최근·즐겨찾기 대상은 각각 최대
32개를 전용 원자적 `~/.hamn/tui.json`에 저장하며 명령·자격 증명은 저장하지 않습니다.
VM 상태 상세에서 공유·번역 설정을 볼 수 있습니다.
