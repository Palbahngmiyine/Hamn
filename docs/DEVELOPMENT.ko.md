...
별도 runner에서 실행합니다. 샤드 안에서 `build/hamn`을 다시 빌드하지 않는 검증은
side lane(`CI_MACOS_SIDE_GATES`)과 updater lane(`CI_MACOS_SHARD_<n>_UPDATER`)에서
main lane과 동시에 실행하고, 다른 버전을 빌드하는 검증(`CI_MACOS_ALONE_GATES`)은
마지막에 실행합니다. PR CI는
