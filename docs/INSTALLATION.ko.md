...
처음 설치하거나 호환되지 않는 이전 설치(아래 참고)에는
...
실행하지 마세요. 이전 배치(아래 참고)의 설치본은 정리하지 않습니다.
...
Hamn 0.1.2 이하와 출시 전 빌드로 만든 설치는 generation에 업데이트기 shell
스크립트를 담던 이전 배치(배치 version 1)를 사용합니다. 현재 설치기는 이런
generation을 채택하거나 변경하지 않습니다. `install.sh`와 `make install`은 아무것도
바꾸지 않고 거부하며, 옮겨야 할 명령 링크와 데이터 디렉터리를 알려 줍니다. 두 경로를
옮긴 뒤 공식 설치기를 다시 실행합니다.
...
`~/.hamn` 아래의 프로필, VM 디스크, 게스트 이미지 캐시는 유지됩니다. 새 설치가
정상 동작하면 옮긴 경로를 삭제합니다. Hamn 0.1.2 이하는 새 릴리스에 제자리에서
도달할 수 없습니다(아래 참고). schema v3를 읽는 출시 전 빌드는 새 아카이브에 설치기
스크립트가 없으므로 자체 `hamn upgrade`가 호스트 아티팩트 검증에서 실패합니다. 둘 다
같은 방법으로 다시 설치합니다.
...
제자리에서 업그레이드할 수 없고 위에서 설명한 대로 공식 설치기로 다시 설치해야 합니다. 이미 공개된
과거 릴리스는 수정하지 않습니다.
