...
fn retirement_result(
    ...
    if cancelled {
        return Err(cancellation_failure(error));
    }
    Ok(error)
}
    ...
    match result {
        ...
        Ok(mut value) => {
            if let Some(error) = previous {
                value["migrationError"] = serde_json::json!(error);
            }
            Ok(value)
        }
    }
...
pub async fn execute_stream(
    ...
    if request.mutates()
        && !(request.words.first().is_some_and(|word| word == "docker")
        ...
        )
    {
        ...
        migration_error =
            retirement_result(&request.operation(), migration, cancel.is_cancelled())?;
    }
    if cancel.is_cancelled() {
        return Err(cancellation_failure(migration_error));
    }
    let run = async {
...
mod tests {
    ...
    fn failed_retirement_can_only_proceed_to_stop_and_retains_its_error() {
        for operation in [
            ...
            let result = retirement_result(
                operation,
                Err(Failure::new("coreError", "retirement failed")),
                false,
            );
        ...
        for operation in ["vm stop", "vm delete", "docker containers start"] {
            for cancelled in [false, true] {
                for code in ["outcomeUnknown", "operationFailed", "cancelled"] {
                    let result = retirement_result(
                        operation,
                        Err(Failure::new(code, "retirement diagnostic")),
                        cancelled,
                    );
    ...
    fn cancellation_at_later_dispatch_boundaries_cannot_erase_retirement_uncertainty() {
        ...
        assert_eq!(
            cancellation_failure(previous.clone()).code,
            "outcomeUnknown"
        );
        let result = after_retirement(
            Err(Failure::new("cancelled", "dispatch cancelled")),
            previous.clone(),
        );
        assert_eq!(result.unwrap_err().message, "retirement diagnostic");
        let result = after_retirement(
            Err(Failure::new("outcomeUnknown", "stop diagnostic")),
            previous.clone(),
        );
        assert_eq!(result.unwrap_err().message, "stop diagnostic");
        let result =
            after_retirement(Ok(serde_json::json!({"state":"stopped"})), previous).unwrap();
        assert_eq!(result["migrationError"]["code"], "outcomeUnknown");
        assert_eq!(
            after_retirement(Err(Failure::new("cancelled", "cancelled")), None)
                .unwrap_err()
                .code,
            "cancelled"
        );
