...
impl Workspace {
    pub fn index(self) -> usize {
        if self == Self::Containers { 0 } else { 1 }
    }
}
...
impl Target {
    pub fn label(&self) -> String {
        match self {
            ...
            Self::Docker { name, config } => format!(
                "Docker context: {name} | config: {}",
                config.as_deref().unwrap_or("CLI default")
            ),
            Self::Kubernetes {
                name,
                namespace,
                ...
                "Kubernetes: {name} / {} | kubeconfig: {}",
                namespace.as_deref().unwrap_or("context default"),
                config.as_deref().unwrap_or("CLI default")
            ),
        }
    }
    fn valid(&self) -> bool {
        ...
        !fields[0].is_empty()
            && fields
                .iter()
                .all(|s| s.len() <= 1024 && !s.chars().any(char::is_control))
    }
}
fn invalid() -> io::Error {
    io::Error::other("Invalid or unsafe ~/.hamn/tui.json; choose a default workspace to replace it")
}
...
impl Drop for UpdateLock {
    fn drop(&mut self) {
        // Another thread may fork a PTY child while this descriptor is open.
        // CLOEXEC closes it at exec, but cannot release an inherited flock
        // before exec. End our critical section explicitly on the shared open
        // file description so an unrelated child cannot keep preferences busy.
        loop {
            ...
            assert_eq!(
                ...
                "cannot unlock TUI preferences: {error}"
            );
        }
    }
}
    ...
    loop {
        ...
        if error.kind() == io::ErrorKind::WouldBlock {
            return Err(io::Error::new(
                ...
                "TUI preferences are busy in another instance; retry after it finishes",
            ));
        }
        return Err(error);
    }
...
fn load_document(path: &Path) -> io::Result<Option<Preferences>> {
    ...
    if value.version != 1
        || value.recent_targets.len() > 32
        || value.favorites.len() > 32
        || !value
            .recent_targets
            .iter()
            .chain(&value.favorites)
            .all(Target::valid)
    {
        return Err(invalid());
    }
...
pub fn load(path: &Path) -> io::Result<Option<Workspace>> {
    Ok(load_document(path)?.map(|value| value.default_workspace))
}
pub fn save(path: &Path, workspace: Workspace) -> io::Result<()> {
    let _lock = update_lock(path)?;
    let mut value = load_document(path).ok().flatten().unwrap_or(Preferences {
        version: 1,
        default_workspace: workspace,
...
pub fn targets(path: &Path, favorites: bool) -> io::Result<Vec<Target>> {
    Ok(load_document(path)?.map_or(Vec::new(), |value| {
        if favorites {
            value.favorites
        } else {
            value.recent_targets
        }
    }))
}
...
fn remember_target_locked(
    ...
    let values = if favorite {
        &mut value.favorites
    } else {
        &mut value.recent_targets
    };
    let existing = values.iter().position(|saved| saved == &target);
    if let Some(index) = existing {
        values.remove(index);
    }
    if !favorite || existing.is_none() {
...
mod tests {
    ...
    #[test]
    fn forked_child_cannot_extend_a_completed_preference_update_lock() {
        let root =
            std::env::temp_dir().join(format!("hamn-preference-fork-{}", std::process::id()));
        ...
        assert!(child >= 0);
        if child == 0 {
            // The multithreaded test runner's child uses only async-signal-safe
            // libc operations before _exit; it never enters Rust allocation.
            unsafe {
                libc::close(pipe[1]);
                ...
                let ready = libc::poll(&mut poll, 1, 5000);
                libc::_exit(if ready > 0 { 0 } else { 1 });
            }
        }
        unsafe {
            libc::close(pipe[0]);
        ...
        drop(lock);
        let result = save(&path, Workspace::Kubernetes);
        unsafe {
            libc::close(pipe[1]);
        }
        let mut status = 0;
        ...
        result.unwrap();
        assert_eq!(load(&path).unwrap(), Some(Workspace::Kubernetes));
        fs::remove_dir_all(root).unwrap();
    }
    #[test]
    fn target_labels_distinguish_same_context_in_different_configuration_files() {
        let first = Target::Docker {
            name: "dev".into(),
            config: Some("/first/config".into()),
        };
        let second = Target::Docker {
            name: "dev".into(),
            config: Some("/second/config".into()),
        };
        assert_ne!(first.label(), second.label());
        let first = Target::Kubernetes {
            name: "dev".into(),
            namespace: None,
            config: Some("/first/kubeconfig".into()),
        };
        let second = Target::Kubernetes {
            name: "dev".into(),
            namespace: None,
            config: Some("/second/kubeconfig".into()),
        };
        assert_ne!(first.label(), second.label());
    ...
    fn preference_lock_rejects_links_unsafe_permissions_and_non_files() {
        let root =
            std::env::temp_dir().join(format!("hamn-preference-lock-{}", std::process::id()));
        fs::create_dir(&root).unwrap();
        let path = root.join(".hamn/tui.json");
        save(&path, Workspace::Containers).unwrap();
        let lock = root.join(".hamn/tui.lock");
    ...
    fn target_writer_fixture() {
        ...
        if first {
            let lock = update_lock(&path).unwrap();
            println!("WRITER_LOCKED");
            io::stdout().flush().unwrap();
            io::stdin().read_exact(&mut [0u8]).unwrap();
            remember_target_locked(
                &path,
                Target::Hamn {
                    name: "first".into(),
                },
            ...
            let target = Target::Hamn {
                name: "second".into(),
            };
            assert_eq!(
                remember_target(&path, target.clone(), true)
                    .unwrap_err()
                    .kind(),
                io::ErrorKind::WouldBlock
            );
        ...
        impl Writer {
            fn start(path: &Path, which: &str) -> Self {
                ...
                let mut child = std::process::Command::new(std::env::current_exe().unwrap())
                    .args([
                        "--exact",
                        "preferences::tests::target_writer_fixture",
                        "--ignored",
                        "--nocapture",
                    ])
                    .env("HAMN_PREFERENCES_TEST_PATH", path)
                    .env("HAMN_PREFERENCES_TEST_WRITER", which)
            ...
            fn release(&mut self) {
                self.child.stdin.as_mut().unwrap().write_all(b"1").unwrap();
            }
        }
        let root = std::env::temp_dir().join(format!(
            "hamn-concurrent-preferences-{}",
            std::process::id()
        ));
        fs::create_dir(&root).unwrap();
        let path = root.join(".hamn/tui.json");
        save(&path, Workspace::Containers).unwrap();
        let mut first = Writer::start(&path, "first");
        first.until("WRITER_LOCKED");
        let mut second = Writer::start(&path, "second");
        second.until("WRITER_BUSY");
        assert!(targets(&path, false).unwrap().is_empty());
        assert!(targets(&path, true).unwrap().is_empty());
        first.release();
        first.until("WRITER_COMMITTED");
        assert!(first.child.wait().unwrap().success());
        second.release();
        second.until("WRITER_COMMITTED");
        assert!(second.child.wait().unwrap().success());
        assert_eq!(
            targets(&path, false).unwrap(),
            vec![Target::Hamn {
                name: "first".into()
            }]
        );
        assert_eq!(
            targets(&path, true).unwrap(),
            vec![Target::Hamn {
                name: "second".into()
            }]
        );
    ...
    fn targets_are_bounded_atomic_private_and_preserved_by_workspace_changes() {
        let root =
            std::env::temp_dir().join(format!("hamn-target-preferences-{}", std::process::id()));
        fs::create_dir(&root).unwrap();
        let path = root.join(".hamn/tui.json");
        save(&path, Workspace::Containers).unwrap();
        ...
        assert!(
            serde_json::from_str::<Target>(r#"{"kind":"hamn","name":"dev","token":"secret"}"#)
                .is_err()
        );
        fs::remove_dir_all(root).unwrap();
    }
    ...
    fn preferences_roundtrip_reject_corruption_and_never_follow_links() {
        ...
        for text in [
            ...
            "{}",
            "null",
            "{\"version\":2,\"defaultWorkspace\":\"containers\"}",
            "{\"version\":1,\"defaultWorkspace\":\"vm\"}",
            "bad",
            "{\"version\":1,\"version\":1}",
        ] {
