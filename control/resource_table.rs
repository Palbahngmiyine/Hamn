...
pub fn draw(frame: &mut Frame, area: Rect, state: &State, title: String) {
    ...
    let columns: &[&str] = if invocation.workspace == Workspace::Kubernetes { kube_columns(invocation.resource.as_deref().unwrap_or("")) }
    ...
    let offset = state.selected.unwrap_or(0).saturating_sub(visible.saturating_sub(1));
    let now = jiff::Timestamp::now();
    let rows: Vec<Row> = state.rows().into_iter().skip(offset).take(visible).map(|row| {
        let fields: Vec<Cell> = if invocation.workspace == Workspace::Kubernetes {
            kube_fields(row, invocation.resource.as_deref().unwrap_or(""), now).into_iter().map(Cell::from).collect()
        } else {
    ...
    frame.render_stateful_widget(table, area, &mut TableState::default().with_selected(state.selected.map(|index| index - offset)));
}
fn kube_columns(resource: &str) -> &'static [&'static str] {
    match resource {
        ...
        "deployments" | "deployment" | "deploy" | "statefulsets" | "statefulset" | "sts" | "daemonsets" | "daemonset" | "ds" => &["Name", "Namespace", "Ready", "Updated", "Available"],
        _ => &["Name", "Namespace", "Status"],
    }
}
fn count(value: &Value) -> u64 { value.as_u64().unwrap_or(0) }
fn age(created: &Value, now: jiff::Timestamp) -> String {
    let Some(created) = created.as_str().and_then(|s| s.parse::<jiff::Timestamp>().ok()) else { return "?".into(); };
    let seconds = now.as_second().saturating_sub(created.as_second()).max(0);
    if seconds >= 86400 { format!("{}d", seconds / 86400) }
    else if seconds >= 3600 { format!("{}h", seconds / 3600) }
    else if seconds >= 60 { format!("{}m", seconds / 60) }
...
fn pod_reason(row: &Value) -> String {
    if !row["metadata"]["deletionTimestamp"].is_null() { return "Terminating".into(); }
    if let Some(reason) = row["status"]["reason"].as_str().filter(|s| !s.is_empty()) { return text(&Value::from(reason)); }
    for field in ["initContainerStatuses", "containerStatuses"] {
        for status in row["status"][field].as_array().into_iter().flatten() {
            if let Some(reason) = status["state"]["waiting"]["reason"].as_str() { return text(&Value::from(reason)); }
            if let Some(reason) = status["state"]["terminated"]["reason"].as_str() {
                if count(&status["state"]["terminated"]["exitCode"]) != 0 { return text(&Value::from(reason)); }
            }
        }
    }
...
fn kube_fields(row: &Value, resource: &str, now: jiff::Timestamp) -> Vec<String> {
    let mut fields = vec![text(&row["metadata"]["name"]), text(&row["metadata"]["namespace"])];
    match resource {
        "pods" | "pod" | "po" => {
            let statuses: Vec<_> = row["status"]["containerStatuses"].as_array().into_iter().flatten().collect();
            let total = row["spec"]["containers"].as_array().map_or(statuses.len(), Vec::len);
            let ready = statuses.iter().filter(|s| s["ready"] == true).count();
            let restarts: u64 = ["containerStatuses", "initContainerStatuses"].iter().flat_map(|field| row["status"][field].as_array().into_iter().flatten()).map(|s| count(&s["restartCount"])).fold(0, u64::saturating_add);
            fields.extend([format!("{ready}/{total}"), restarts.to_string(), pod_reason(row), age(&row["metadata"]["creationTimestamp"], now)]);
        },
        "events" | "event" | "ev" => fields.extend([text(&row["type"]), text(&row["reason"]), text(if row["message"].is_null() { &row["note"] } else { &row["message"] })]),
        "nodes" | "node" | "no" => {
            let conditions: Vec<_> = row["status"]["conditions"].as_array().into_iter().flatten().collect();
            let ready = conditions.iter().find(|c| c["type"] == "Ready").map_or("Unknown".into(), |c| text(&c["status"]));
            let mut issues: Vec<_> = conditions.iter().filter(|c| c["type"] != "Ready" && c["status"] == "True").map(|c| text(&c["type"])).collect();
            if row["spec"]["unschedulable"] == true { issues.push("SchedulingDisabled".into()); }
            fields.extend([ready, issues.join(", ")]);
        },
        "deployments" | "deployment" | "deploy" | "statefulsets" | "statefulset" | "sts" | "daemonsets" | "daemonset" | "ds" => {
            let daemon = matches!(resource, "daemonsets" | "daemonset" | "ds");
            let desired = if daemon { count(&row["status"]["desiredNumberScheduled"]) } else { row["spec"]["replicas"].as_u64().unwrap_or(1) };
            let ready = count(&row["status"][if daemon { "numberReady" } else { "readyReplicas" }]);
            fields.extend([format!("{ready}/{desired}"), count(&row["status"][if daemon { "updatedNumberScheduled" } else { "updatedReplicas" }]).to_string(), count(&row["status"][if daemon { "numberAvailable" } else { "availableReplicas" }]).to_string()]);
        },
        _ => fields.push(if !row["status"]["phase"].is_null() { text(&row["status"]["phase"]) }
...
mod tests {
    ...
    fn viewport_preserves_global_selection_at_the_end_of_large_filtered_lists() {
        ...
        let mut terminal = ratatui::Terminal::new(ratatui::backend::TestBackend::new(120, 10)).unwrap();
        terminal.draw(|frame| draw(frame, frame.area(), &state, "pods".into())).unwrap();
        ...
        terminal.draw(|frame| draw(frame, frame.area(), &state, "filtered".into())).unwrap();
        let rendered: String = terminal.backend().buffer().content.iter().map(|cell| cell.symbol()).collect();
    ...
    fn pod_diagnostics_do_not_treat_running_phase_as_healthy() {
        ...
        assert_eq!(kube_fields(&pod, "pods", now), ["bad","work","1/2","42","CrashLoopBackOff","1h"]);
        pod["status"]["containerStatuses"][0]["state"] = serde_json::json!({"waiting":{"reason":"ImagePullBackOff"}});
        assert_eq!(pod_reason(&pod), "ImagePullBackOff");
        pod["metadata"]["deletionTimestamp"] = "now".into(); assert_eq!(pod_reason(&pod), "Terminating");
    }
    #[test]
    fn events_nodes_and_workloads_expose_actionable_fields() {
        ...
        assert_eq!(&kube_fields(&event,"events",now)[2..], ["Warning","FailedScheduling","Insufficient memory"]);
        ...
        assert_eq!(&kube_fields(&node,"nodes",now)[2..], ["False","MemoryPressure, SchedulingDisabled"]);
        let deployment = serde_json::json!({"spec":{"replicas":3},"status":{"readyReplicas":1,"updatedReplicas":2,"availableReplicas":1}});
        assert_eq!(&kube_fields(&deployment,"deployments",now)[2..], ["1/3","2","1"]);
    }
    #[test]
