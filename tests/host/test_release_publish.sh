...
grep -Fq '"workflow":{"attempt":"local","run":"local"}' \
    "$INPUT/hamn-evidence/validation-evidence.json"
...
HAMN_RELEASE_SIGNING_KEY="$WORK/release-key" \
HAMN_RELEASE_REPOSITORY="$RELEASE_REPOSITORY" \
HAMN_EXPECTED_WORKFLOW_RUN=1 \
    bash "$ROOT/packaging/release/publish-release.sh" \
    v0.0.1 v0.0.1-rc.1 "$RELEASE_REF" "$INPUT" "$LOCAL_WITH_RUN" \
    >"$WORK/local-with-run.out" 2>"$WORK/local-with-run.err"; then
    echo "FAIL: local publish accepted a workflow run input" >&2
    exit 1
fi
grep -Fq 'release promotion must not accept workflow run inputs' \
...
HAMN_RELEASE_REPOSITORY="$RELEASE_REPOSITORY" \
    bash "$ROOT/packaging/release/publish-release.sh" \
    v0.0.1 v0.0.1-rc.1 "$RELEASE_REF" "$INPUT" "$LOCAL_IN_ACTIONS" \
    >"$WORK/local-in-actions.out" 2>"$WORK/local-in-actions.err"; then
    echo "FAIL: local publish ran inside GitHub Actions" >&2
    exit 1
fi
grep -Fq 'release promotion is unavailable inside GitHub Actions' \
    "$WORK/local-in-actions.err"

LOCAL_WITH_PROVENANCE=$WORK/local-with-provenance
mkdir "$LOCAL_WITH_PROVENANCE"
if HAMN_VALIDATOR_PUBLIC_KEY="$WORK/validator-key.pub" \
HAMN_RELEASE_SIGNING_KEY="$WORK/release-key" \
HAMN_RELEASE_REPOSITORY="$RELEASE_REPOSITORY" \
HAMN_RELEASE_PROVENANCE=solo-local \
    bash "$ROOT/packaging/release/publish-release.sh" \
    v0.0.1 v0.0.1-rc.1 "$RELEASE_REF" "$INPUT" "$LOCAL_WITH_PROVENANCE" \
    >"$WORK/local-with-provenance.out" 2>"$WORK/local-with-provenance.err"; then
    echo "FAIL: local publish accepted a provenance selector" >&2
    exit 1
fi
grep -Fq 'HAMN_RELEASE_PROVENANCE must not be set; release promotion is local only' \
    "$WORK/local-with-provenance.err"
