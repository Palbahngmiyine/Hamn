...
const BUILD_CANDIDATE: &str = "packaging/release/build-candidate.sh";
...
pub fn release_fixtures_do_not_inherit_workflow_identity() {
    for release_test in [
        ...
        "tests/host/test_release_publish.sh",
    ] {
        assert!(
...
pub fn hosted_evidence_does_not_overstate_validation() {
    ...
    assert!(
        contains("packaging/release/hosted-validation.sh", r#""$HAMN_DEV" release hosted-evidence"#),
...
pub fn keyless_promotion_binds_candidates_and_manifest() {
    ...
    assert!(
        contains(
            BUILD_CANDIDATE,
            r#"CANONICAL_MANIFEST_URL="https://github.com/${RELEASE_REPOSITORY}/releases/latest/download/hamn-update-manifest-v3.json""#
        ),
...
pub fn release_path_uses_no_long_lived_signature() {
    assert_absent(
        ...
        &[BUILD_CANDIDATE, INSTALLER, PUBLISH, UPDATER, RELEASE_WORKFLOW],
        "keyless release path still depends on a long-lived release signature",
    );
    assert_absent(
        "ssh-keygen -Y sign",
        &[BUILD_CANDIDATE, INSTALLER, PUBLISH, UPDATER],
