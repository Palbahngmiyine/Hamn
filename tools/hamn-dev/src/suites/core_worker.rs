...
//! state the previous steps left. The script's managed-K3s steps (a
//! `kubernetes:` configuration section, `migration` status and `vm migrate`)
//! and its legacy `prev_*_context` state record are not ported: those
...
fn core_worker_isolation_and_protocol() {
    ...
    vm_stop_does_not_run_external_clis(&mut worker);
...
/// Stopping never runs the external Docker or Kubernetes CLI: user-selected
/// contexts are not Hamn's to change. `docker` and `kubectl` are fixtures
/// that record a run and exit 99; the PATH change stays in effect for the
/// rest of the flow.
fn vm_stop_does_not_run_external_clis(worker: &mut Worker) {
