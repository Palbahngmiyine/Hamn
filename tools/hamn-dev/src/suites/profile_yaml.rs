...
//! settings survive resource-only edits, malformed or legacy profiles are
//! refused, and deletion is soft.
//!
//! One flow, as in the script it replaces. The legacy step (a pre-YAML
//! `hamn.conf` profile) is `legacy_hamn_conf_profile_is_refused`.
...
fn strict_profiles_advanced_settings_preservation_and_soft_deletion() {
    ...
    legacy_hamn_conf_profile_is_refused(&profiles, root);
...
/// A profile left from the pre-YAML format (`hamn.conf`) is neither read nor
/// converted.
fn legacy_hamn_conf_profile_is_refused(profiles: &Profiles, root: &Path) {
    let legacy = root.join(".hamn/legacy");
    fs::create_dir(&legacy).unwrap();
    fs::write(legacy.join("hamn.conf"), "runtime=containerd\n").unwrap();
    assert!(!profiles.ok(&["vm", "status", "--profile", "legacy"]));
    assert!(!profiles.ok(&["vm", "start", "--profile", "legacy", "--yes"]));
